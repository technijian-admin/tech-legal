"""
Culp Construction Company (CCC) Executive Summary Generator
Home Computer Support — Desktop Security + Email + Virtual Staff
...
def section_header(title):
    """Section header with blue bottom border accent."""
    p = doc.add_paragraph()
    ...
    pBdr = parse_xml(
        f'<w:pBdr {nsdecls("w")}>'
        f'<w:bottom w:val="single" w:sz="8" w:space="2" w:color="{HEX_BLUE}"/>'
        "</w:pBdr>"
    )
    pPr.append(pBdr)
    return p
...
CLIENT_NAME = "Culp Construction Company"
CLIENT_SHORT = "CulpCo"
CLIENT_CODE = "CCC"
CLIENT_WEBSITE = "culpco.com"

USERS = 1
DESKTOPS = 1      # 1 home computer
LOCATIONS = 1     # home office

# ── Per-Unit Monthly Pricing (from PriceList) ──
...
PRICE_M365_BS = 15.00          # M365 Business Standard per user

# Support rates — custom: India only (no US tech support, no CTO advisory)
...
# Support hours — custom: 1 hr day + 1 hr night (India only)
INDIA_NIGHT_HOURS = 1.0       # 1 hr India night shift (US biz hours)
INDIA_DAY_HOURS = 1.0         # 1 hr India day shift (US after-hours)
TOTAL_SUPPORT_HOURS = 2.0
...
cost_m365 = USERS * PRICE_M365_BS
subtotal_email = cost_dmarc + cost_m365

# Virtual Staff (India only)
cost_india_night = INDIA_NIGHT_HOURS * RATE_INDIA_NIGHT
cost_india_day = INDIA_DAY_HOURS * RATE_INDIA_DAY
subtotal_support = cost_india_night + cost_india_day
...
p.alignment = WD_ALIGN_PARAGRAPH.CENTER
p.paragraph_format.space_after = Pt(0)
run_styled(p, "May 2026", size=11, color=BRAND_GREY)
...
services_justification = [
    ("CrowdStrike Falcon EDR",
     ...
     f"With {DESKTOPS} desktops handling sensitive bid documents, project financials, and "
     f"subcontractor records, {CLIENT_NAME} needs real-time threat detection that goes beyond "
     "traditional antivirus. CrowdStrike uses AI-driven behavioral analysis to detect ransomware, "
     "fileless attacks, and zero-day exploits before they execute. Construction companies "
     "are increasingly targeted by ransomware groups due to the volume of financial "
     "data they process \u2014 bid proposals, payment applications, and subcontractor agreements."),
    ...
    ("Huntress Managed Detection",
     ...
     "threat intelligence \u2014 critical for an environment handling project financials and bid documents."),
    ...
    ("Cisco Umbrella (My Secure Internet)",
     ...
     f"for all {DESKTOPS} of {CLIENT_NAME}\u2019s desktops, especially important when accessing "
     "cloud applications (M365, construction management software), project files, and bid management tools."),
    ...
    ("DMARC/DKIM Monitoring",
     ...
     f"{CLIENT_NAME}\u2019s domain \u2014 targeting clients, subcontractors, and partners with "
     "fraudulent invoices or change-order requests. Ongoing monitoring ensures email authentication "
    ...
    ("Microsoft 365 Business Standard",
     f"Cloud productivity suite for {CLIENT_NAME}\u2019s mailbox, including Exchange Online, "
     "OneDrive, SharePoint, and Teams.",
     "Microsoft 365 Business Standard provides the full Office suite plus enterprise-grade email "
     "with 50 GB mailbox storage, OneDrive cloud storage, and Teams collaboration. Technijian "
     "manages the license, mailbox configuration, and ongoing administration so Chuck can focus "
     "on running the business without worrying about email infrastructure or license renewals."),

    ("Offshore Virtual Staff Support",
     f"2 hours/month of India-based remote technical support for {CLIENT_NAME}\u2019s "
     "home computer and Microsoft 365 environment.",
     "Dedicated offshore support covers software troubleshooting, Windows updates, M365 configuration, "
     "printer issues, and general desktop support. 1 hour during US business hours (India night shift "
     "at $30/hr) and 1 hour during US after-hours (India day shift at $15/hr) provides coverage for "
     "both routine daytime requests and after-hours maintenance without US-rate overhead."),
]
...
styled_table(
    ...
    [
        ...
        ("M365 Business Standard", "Per user", str(USERS), f"${PRICE_M365_BS:.2f}", f"${cost_m365:,.2f}"),
        ("SUBTOTAL \u2014 EMAIL & DOMAIN", "", "", "", f"${subtotal_email:,.2f}"),
    ],
    col_widths=[2.2, 1.0, 0.5, 0.9, 1.0],
    total_row_indices=[2],
)
...
styled_table(
    ...
    [
        ...
        ("SUBTOTAL \u2014 VIRTUAL STAFF", "", "", f"{TOTAL_SUPPORT_HOURS:.0f}", "", f"${subtotal_support:,.2f}"),
    ],
...
OUTPUT = os.path.join(os.path.dirname(__file__), "..", "02_MSA", "CCC-Executive-Summary.docx")
doc.save(OUTPUT)
