/**
 * Brandywine Homes (BWH) — MSA Signing Document
 * Output: MSA-BWH-Agreement.docx
 * Contains: Cover + MSA Sections 1-11 + Signature Block
 * Schedules are attached separately.
 */
...
// Content width for 0.75" margins: 12240 - 2*1080 = 10080 DXA
const CW = 10080;

// Override brand-helpers with correct width for narrow margins
function sectionHeader(numberOrTitle, title) {
  const text = title ? `${numberOrTitle}. ${title.toUpperCase()}` : numberOrTitle;
  return new Table({
    ...
    rows: [new TableRow({
      children: [
        ...
        new TableCell({ borders: noBorders, width: { size: CW - 120, type: WidthType.DXA },
          margins: { top: 60, bottom: 60, left: 160, right: 0 },
          children: [new Paragraph({
            children: [new TextRun({ text, font: "Open Sans", size: 24, bold: true, color: CORE_BLUE })] })] }),
      ]
    })]
  });
}

// Override numberedSectionHeader to match
const numberedSectionHeader = sectionHeader;

// Override colorBanner for full width
...
// Override accentBar for full width
...
function numbered(num, text, indent) {
  ...
  return new Paragraph({
    ...
    children: [
      new TextRun({ text: `${prefix}${num} `, font: "Open Sans", size: 20, bold: true, color: CORE_BLUE }),
  ...
  return new Table({
    ...
    rows: [new TableRow({
      children: [new TableCell({
        ...
        children: [new Paragraph({
          children: [
            new TextRun({ text: title, font: "Open Sans", size: 20, bold: true, color: DARK_CHARCOAL }),
...
function sigLine(label, anchor) {
  return new Paragraph({
    ...
    children: [
      ...
      new TextRun({ text: label, font: "Open Sans", size: 20, color: BRAND_GREY }),
    ]
  });
}
...
const doc = new Document({
  ...
  sections: [
    // ── COVER PAGE ──
    {
      ...
      children: [
        // Clean legal cover page — no orange divider, no CTA, no sales elements
        accentBar(CORE_BLUE),
        ...
        new Paragraph({ spacing: { before: 400 }, alignment: AlignmentType.CENTER,
          children: [new TextRun({ text: "Master Service Agreement", font: "Open Sans", size: 52, bold: true, color: DARK_CHARCOAL })] }),
        new Paragraph({ spacing: { before: 200 }, alignment: AlignmentType.CENTER,
          children: [new TextRun({ text: "Managed IT Services, Cybersecurity & Virtual Staff", font: "Open Sans", size: 26, color: BRAND_GREY })] }),
        new Paragraph({ spacing: { before: 100 }, alignment: AlignmentType.CENTER,
          children: [new TextRun({ text: "Prepared for Brandywine Homes", font: "Open Sans", size: 26, bold: true, color: CORE_BLUE })] }),
        new Paragraph({ spacing: { before: 200 }, alignment: AlignmentType.CENTER,
          children: [new TextRun({ text: EFFECTIVE_DATE, font: "Open Sans", size: 20, color: BRAND_GREY })] }),
        new Paragraph({ spacing: { before: 2400 }, children: [] }),
        accentBar(CORE_BLUE),
        new Paragraph({ spacing: { before: 120 }, alignment: AlignmentType.CENTER,
          children: [new TextRun({ text: "CONFIDENTIAL", font: "Open Sans", size: 18, italics: true, color: BRAND_GREY })] }),
      ]
    },

    // ── MSA BODY (Sections 1-11) + Signatures ──
    {
      properties: {
        page: {
          size: { width: 12240, height: 15840 },
      ...
      children: [
        ...
        spacer(40),
        infoBox(
          "TERM NOTICE \u2014 NO AUTOMATIC RENEWAL:",
        ...
        spacer(),
        bodyText("Schedules (attached separately):"),
...
// ── Generate DOCX ──
const outPath = path.join(__dirname, '..', '02_MSA', 'MSA-BWH-Agreement.docx');
