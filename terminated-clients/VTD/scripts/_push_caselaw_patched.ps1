...
# Read patched HTML with UTF-8 encoding
$patchedHtml = [System.IO.File]::ReadAllText("C:\VSCode\tech-legal\tech-legal\terminated-clients\VTD\_working_caselaw_draft_patched.html", [System.Text.Encoding]::UTF8)

$patchBody = @{ body = @{ contentType = "HTML"; content = $patchedHtml } } | ConvertTo-Json -Depth 5
Invoke-MgGraphRequest -Method PATCH `
    -Uri "https://graph.microsoft.com/v1.0/users/$senderUpn/messages/$messageId" `
    -Body $patchBody `
    -Headers @{ "Content-Type" = "application/json" } | Out-Null
Write-Host "Draft body replaced with patched HTML." -ForegroundColor Green
