# Draft a reply to Jodie Phan's 4/30/2026 "Invoices and Monthly Time Entries" email
# explaining the rebuild, the billing mechanics, and Vintage Design document logistics.
# DRAFTS ONLY — does NOT call /send. User reviews and sends manually.
...
# Jodie Phan's "Invoices and Monthly Time Entries" email from 4/30/2026 21:13 PT
$jodieMsgId = 'AAMkAGNlYjM0OTA4LThjMjYtNGQ3My1iNDg1LTQ2MTI5NTg0NzFlOQBGAAAAAAC88IffM67WS4tSyVwwqYmJBwBhk-ls8ubYRazD3tGgncxCAAAAAAENAACgx7VhNWW1QYCgfGa-8kbOAAYFY0NdAAA='
...
<br>&nbsp;&nbsp;&bull; <strong>June 2024 online services</strong> (AVD, AVMS, MR, PMW, SI, OPS-NET, DKIM, etc.) — current month
<br>&nbsp;&nbsp;&bull; <strong>July 2024 support hours</strong> (offshore normal, offshore after-hours, onshore normal at the prior cycle&rsquo;s monthly average) — advance month
...
<p>Each monthly invoice email also contained a "Monthly Time Entries for [InvoiceID].xlsx" attachment. That spreadsheet reports the <strong>actual hours worked during the prior month</strong> &mdash; in arrears. It is a record of what already happened, complementing the forward-looking invoice.</p>

<p>Continuing the same example:</p>

<blockquote style="border-left: 3px solid #ccc; padding-left: 12px; color: #333;">
The June 1, 2024 invoice email (transmitted June 2, 2024) carried a "Monthly Time Entries for [&hellip;].xlsx" attachment that detailed every ticket time entry recorded for <strong>May 2024 support work</strong>. Each row shows Title, Date Requested, Requestor, Start, End, Normal Qty (column H), After-Hours Qty (column J), and POD (column N: CHD-TS1 = offshore, IRV-TS1 = onshore).
</blockquote>
...
<p>In addition to the monthly cadence, Technijian sent weekly emails. Each weekly email carried a "Weekly Time Entries for [InvoiceID].xlsx" attachment listing every time entry recorded during that week. These weekly files are an incremental running record of work performed.</p>
...
<p>The reconciliation rebuild and the cancellation-fee math include only Contract = "Under Contract" line items, which is the correct scope for ¶¶ 3-5 of the Client Monthly Service Agreement.</p>
...
<h3 style="margin-bottom: 4px;">6. Direct answers to your questions</h3>
...
<p>The specific May 2023 anomaly you flagged ("the Excel summary shows only 2.75 hours" of onshore Tech Services) was a consequence of that first-pass exclusion. With the rebuild, May 2023 onshore (Tech_Support.R) actuals are 33.72 hours, drawn from the time-entry xlsx attached to the June 2, 2023 monthly invoice email — the first arrears reporting month, consistent with the convention.</p>
...
<br><br>Each monthly invoice email also included a "Monthly Time Entries" spreadsheet detailing the actual hours of support work performed during the prior month. Time entries logged by techs after the monthly cutoff appear on subsequent weekly time-entry spreadsheets, also transmitted to Vintage Design.
...
<table border="1" cellpadding="6" cellspacing="0" style="border-collapse: collapse; font-size: 10pt;">
  <thead>
    <tr style="background-color: #f2f2f2;">
      ...
      <th align="right">$ × $150/hr</th>
      <th align="right">+ 10% late</th>
    </tr>
  </thead>
  <tbody>
    <tr><td>Cycle-as-mechanism: (P2 actual + P3 actual) − P3 billed</td><td align="right">2,472.02</td><td align="right">$370,803.00</td><td align="right">$407,883.30</td></tr>
...
<h3 style="margin-bottom: 4px;">8. Averaging math &mdash; favorable to us</h3>
...
<table border="1" cellpadding="6" cellspacing="0" style="border-collapse: collapse; font-size: 10pt;">
  ...
  <tbody>
    <tr><td>P1 → P2</td><td>OffShore Normal</td><td align="right">81.86</td><td align="right">76.77</td><td align="right">−5.10</td></tr>
    <tr><td>P1 → P2</td><td>OffShore After-Hours</td><td align="right">62.56</td><td align="right">64.48</td><td align="right">+1.92</td></tr>
    <tr><td>P1 → P2</td><td>Tech Support</td><td align="right">21.47</td><td align="right">20.00</td><td align="right">−1.47</td></tr>
    <tr><td>P2 → P3</td><td>OffShore Normal</td><td align="right">114.87</td><td align="right">96.35</td><td align="right">−18.52</td></tr>
    <tr><td>P2 → P3</td><td>OffShore After-Hours</td><td align="right">91.60</td><td align="right">77.66</td><td align="right">−13.94</td></tr>
    <tr><td>P2 → P3</td><td>Tech Support</td><td align="right">15.68</td><td align="right">20.00</td><td align="right">+4.33</td></tr>
  </tbody>
</table>

<p>Stambuk's premise is that the averaging mechanism in ¶¶ 3-4 "fully collected" Period 1's excess via Period 2's elevated billing rate. The data shows Period 2's monthly billed quantity for offshore support was actually below Period 1's actual monthly average. The averaging under-billed Vintage Design relative to the contract math, so the "fully collected" theory does not survive contact with the data.</p>

<h3 style="margin-bottom: 4px;">9. Spreadsheet completeness</h3>
...
# Sanity: confirm currency literals preserved (single-quoted here-string preserves $ literals)
$dollarHits = ([regex]::Matches($replyHtml, '\$\d')).Count
"Dollar literals in body: $dollarHits"
if ($dollarHits -lt 10) { throw "ABORT: dollar count too low ($dollarHits) - currency may have been stripped" }

# Create reply draft (preserves threading)
$draft = Invoke-MgGraphRequest -Method POST -Uri "https://graph.microsoft.com/v1.0/users/RJain@technijian.com/messages/$jodieMsgId/createReply"
"Draft created: $($draft.id)"

# Prepend the reply HTML to the existing draft body (which already contains the quoted original)
$existingBody = $draft.body.content
if ($existingBody -match '<body[^>]*>') {
    $newBody = $existingBody -replace '(<body[^>]*>)', "`$1$replyHtml"
} else {
    $newBody = $replyHtml + $existingBody
}

# PATCH body. createReply auto-includes original sender in To. Cc remains as createReply set it
# (which copies the original Cc: Ed Susolik + Frank Dunn). Verify after patch.
$patchObj = @{
    body = @{ contentType = 'HTML'; content = $newBody }
} | ConvertTo-Json -Depth 10 -Compress

Invoke-MgGraphRequest -Method PATCH -Uri "https://graph.microsoft.com/v1.0/users/RJain@technijian.com/messages/$($draft.id)" `
    -Body $patchObj -ContentType 'application/json'

# Attach the two xlsx files
$rebuildDir = 'c:\VSCode\tech-legal\tech-legal\terminated-clients\VTD\exhibits\rebuild'
$attachments = @(
    @{ path = Join-Path $rebuildDir 'vtd_actual_vs_billed_REBUILT.xlsx'; name = 'VTD_Actual_vs_Billed_Reconciliation.xlsx' },
    @{ path = Join-Path $rebuildDir 'vtd_missing_entries.xlsx';          name = 'VTD_Missing_Entries_Detail.xlsx' }
)
foreach ($a in $attachments) {
    ...
    Invoke-MgGraphRequest -Method POST `
        -Uri "https://graph.microsoft.com/v1.0/users/RJain@technijian.com/messages/$($draft.id)/attachments" `
        -Body $obj -ContentType 'application/json'
    "Attached: $($a.name) ($([Math]::Round($bytes.Length/1024,1)) KB)"
}

# Verify final draft state
Start-Sleep -Seconds 2
$verify = Invoke-MgGraphRequest -Method GET `
    -Uri "https://graph.microsoft.com/v1.0/users/RJain@technijian.com/messages/$($draft.id)?`$select=subject,toRecipients,ccRecipients,isDraft,hasAttachments,bodyPreview,webLink"
"--- Final draft state ---"
