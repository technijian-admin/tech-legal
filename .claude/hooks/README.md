...
| `session-archive.cjs` | `SessionEnd` | **Durable session capture** — copy the full JSONL transcript to `<vault>/session-archive/<date>_<session-short>.jsonl` AND render a human-readable `.md` version. Idempotent (skips if already archived). Preserves tool calls + context loss never happens. |
