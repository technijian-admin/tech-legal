param(
    [string]$Subject = "Complaint received for Ravi Jain"
)
...
$searchQuery = [uri]::EscapeDataString("`"$Subject`"")
$url = "https://graph.microsoft.com/v1.0/users/$UserMail/messages?`$search=$searchQuery&`$top=20&`$select=id,subject,receivedDateTime,hasAttachments,from"
...
foreach ($m in $resp.value) {
    ...
    foreach ($a in $atts) {
        if ($a.size -lt 5000) { Write-Host ("  Skip small attachment {0} ({1} bytes)" -f $a.name, $a.size) -ForegroundColor DarkGray; continue }
