# BWH MSA package - DocuSign envelope, NATIVE email path, parallel signing
# - 4 documents in single envelope (MSA + Schedules A, B, C)
# - Anchor tabs ONLY on MSA (documentId=1), where signature block + 8 hidden anchors live
# - NO clientUserId on either signer = DocuSign sends its OWN email with PERSISTENT (~120-day) URLs
# - status=sent on creation = DocuSign dispatches the emails immediately
# - Parallel: routingOrder=1 for both Ravi and Dave
...
$docFolder = "C:\vscode\tech-legal\tech-legal\clients\BWH\02_MSA"
$docs = @(
    @{ id = "1"; path = "$docFolder\MSA-BWH-Agreement.docx";              displayName = "MSA - Brandywine Homes" },
    @{ id = "2"; path = "$docFolder\BWH-Schedule-A-Monthly-Services.docx"; displayName = "Schedule A - Monthly Managed Services" },
    @{ id = "3"; path = "$docFolder\BWH-Schedule-B-Subscriptions.docx";    displayName = "Schedule B - Subscription and License Services" },
    @{ id = "4"; path = "$docFolder\BWH-Schedule-C-Rate-Card.docx";        displayName = "Schedule C - Rate Card" }
)
foreach ($d in $docs) {
...
$clientSignerName  = "Dave Barisic"
$clientSignerEmail = "dave@brandywine-homes.com"

$emailSubject = "Brandywine Homes - Master Service Agreement and Schedules - Signature Required"
$emailBlurb   = "Dave - here is the final MSA package we agreed to. Four documents: the MSA, Schedule A (Monthly Managed Services), Schedule B (Subscriptions), and Schedule C (Rate Card). Please review and sign at your convenience. - Ravi"
...
# --- Build documents array (base64 each file) ---
Write-Host "`nReading and encoding 4 documents..." -ForegroundColor Cyan
$docArray = @()
foreach ($d in $docs) {
    ...
    recipients   = @{
        signers = @(
            @{
                ...
                recipientId  = "1"
                routingOrder = "1"
                # NO clientUserId -> DocuSign sends native email with persistent URL
                tabs = @{
...
# Triple-check: confirm zero clientUserId in payload before sending
...
foreach ($d in $docs) { Write-Host "  $($d.id). $($d.displayName)" -ForegroundColor Gray }
Write-Host ""
...
$envelopeId | Set-Content -Path "$scriptDir\bwh-msa-final-envelope-id.txt" -NoNewline
Write-Host "Envelope ID saved to: $scriptDir\bwh-msa-final-envelope-id.txt" -ForegroundColor Gray
