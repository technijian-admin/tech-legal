# Final send — recreate any missing drafts from staging, verify counts, send all 55.
...
# Dedupe by (subject + recipient) pair - only delete TRUE duplicates
$seen = @{}
$dupesDeleted = 0
foreach ($d in ($campaignDrafts | Sort-Object CreatedDateTime -Descending)) {
    ...
    if ($seen.ContainsKey($key)) {
        ...
        $dupesDeleted++
        Write-Host "  removed duplicate: $to / $($d.Subject.Substring(0, [Math]::Min(50, $d.Subject.Length)))"
    } else {
        $seen[$key] = $d.Id
    }
}
...
# Load staging, find which expected drafts are MISSING and recreate them
$staging = Get-Content $stagingPath -Raw | ConvertFrom-Json
$remainingDrafts = Get-MgUserMessage -UserId $senderUpn -Filter "isDraft eq true" -Top 200 -Property "id,subject,toRecipients,hasAttachments"
$existingKeys = @{}
...
# Spot-check: pick one draft per template, verify body hygiene
$samples = @(
...
foreach ($s in $samples) {
    ...
    if ($unmerged.Count -gt 0) { $status = "FAIL: unmerged"; $allOk = $false }
...
if ($finalCampaign.Count -ne 55) {
    Write-Host "COUNT MISMATCH - expected 55 campaign drafts, found $($finalCampaign.Count). ABORTING SEND" -ForegroundColor Red
    Disconnect-MgGraph | Out-Null
    exit 1
}

Write-Host "All 55 drafts verified. Sending..." -ForegroundColor Green
Write-Host ""
