# STAGE 1: Create DocuSign envelope for AFFG SOW-004 Rev 1 using absolute-positioning tabs,
# then RENDER the branded email HTML using the CANONICAL Build-SigningEmail template.
# Does NOT send the Graph email. Writes rendered HTML + state JSON for Ravi's review.
#
# Stage 2 (dispatch-docusign-affg-sow004-rev2.ps1) reads the state JSON and actually sends.
#
# Template source: Build-SigningEmail copied verbatim from send-docusign.ps1 (lines 381-586)
...
$clientPreview   = Join-Path $scriptDir "affg-sow004-rev2-client-preview.html"
$techPreview     = Join-Path $scriptDir "affg-sow004-rev2-tech-preview.html"
...
$userInfo = Invoke-RestMethod -Uri "https://account.docusign.com/oauth/userinfo" -Headers $h
...
# --- Create draft envelope (no tabs yet) ---
...
$d = Invoke-RestMethod -Uri "$apiUrl/envelopes" -Method POST -Headers $hJ -Body $draft -TimeoutSec 60
...
# --- Transition envelope to 'sent' (DocuSign internal; no email because clientUserId suppresses it) ---
...
Write-Host "Envelope transitioned to 'sent' (DocuSign email suppressed). $envelopeId" -ForegroundColor Green
...
$techView = Invoke-RestMethod -Uri "$apiUrl/envelopes/$envelopeId/views/recipient" -Method POST -Headers $hJ -Body (@{
    returnUrl = $returnUrl; authenticationMethod = "email"; email = $SignerEmail; userName = $SignerName; clientUserId = "tech_signer_1"
} | ConvertTo-Json)
...
$clientView = Invoke-RestMethod -Uri "$apiUrl/envelopes/$envelopeId/views/recipient" -Method POST -Headers $hJ -Body (@{
    returnUrl = $returnUrl; authenticationMethod = "email"; email = $RecipientEmail; userName = $RecipientName; clientUserId = "client_signer_2"
} | ConvertTo-Json)
$clientSignUrl = $clientView.url
Write-Host "Signing URLs retrieved." -ForegroundColor Green

# ============================================================
# CANONICAL BRANDED EMAIL TEMPLATE — copied verbatim from send-docusign.ps1 lines 381-586
# ============================================================
...
# --- Render both emails ---
$recipientFirst = ($RecipientName -split '\s+', 2)[0]
...
Set-Content -Path $clientPreview -Value $clientHtml -Encoding UTF8
Set-Content -Path $techPreview   -Value $techHtml   -Encoding UTF8
...
$state = @{
    ...
    clientHtmlFile  = $clientPreview
    signerEmail     = $SignerEmail
    signerName      = $SignerName
    techHtmlFile    = $techPreview
...
Write-Host " STAGE 1 COMPLETE - envelope prepared, no email sent yet" -ForegroundColor Yellow
Write-Host "==========================================="  -ForegroundColor Yellow
Write-Host "Envelope ID          : $envelopeId"
Write-Host "Pages                : $pageCount (tabs on page $lastPage)"
Write-Host "Client preview HTML  : $clientPreview"
Write-Host "Technijian preview   : $techPreview"
Write-Host "State file           : $statePath"
Write-Host ""
Write-Host "Iris has received NOTHING yet. Review the preview HTML."
Write-Host "When approved, run stage 2:" -ForegroundColor Cyan
Write-Host "  powershell.exe -ExecutionPolicy Bypass -File dispatch-docusign-affg-sow004-rev2.ps1" -ForegroundColor Cyan
