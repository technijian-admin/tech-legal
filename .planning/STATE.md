...
- **Phase 9 — Packaging, Deploy & On-Box Smoke** (DEPLOY-01..04): `quickbooks/QbConnectService/deploy/` now contains `_common.ps1`, `make-cert.ps1`, `install-service.ps1`, `uninstall-service.ps1`, and `run-as-task.ps1`, all authored for the live host and statically verified in dev/CI. `SampleConfigParityTests` now enforces the committed `.sample` config surfaces. The operator docs now include `register-integrated-app.md`, the full deploy `README.md`, `SMOKE-CHECKLIST.md`, and `QBXML-REPIN.md`. `Program.cs` now exposes `QbConnectService.exe --verify-audit`, CI includes a `powershell-lint` job, and the remaining QuickBooks skill/readme forward references now point at the real deploy/doc folders. Commits `c46116a`, `d237677`, `91d0706`, `e4ec629`, `d727551`, `407d67c`, `253c372`, the `docs(09-01)` close-out `1d15247`, the reviewer's `fix(09-01)` `c29b7d2` (made `quickbooks/dev/run-codex-phase.ps1` PSScriptAnalyzer-clean so the new broad-scope `powershell-lint` CI job is green; gitignored `quickbooks/QbConnectService/deploy/appsettings.json` + `deploy/*.pfx` + `deploy/*.cer` — a DEPLOY-02 gap the plan missed), and the `docs(review)` section `92986ae`. Zero new NuGet packages. Test count 255→259 (`SampleConfigParityTests` + `VerifyAuditCliTests`). Reviewed 100/100. Remaining work is host-only: interop regen, QuickBooks authorization, live smoke, and live qbXML re-pin.

**Project complete — all 9 phases reviewed at 100/100. Branch `quickbooks/direct-sdk-integration-2026-05-11` is ready for a PR; the live deploy + smoke + re-pin are operator tasks on `10.120.254.13`.**
